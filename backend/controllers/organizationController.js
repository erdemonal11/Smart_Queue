...
const downloadOrganizationData = async (req, res) => {
  try {
    ...
    const organizationData = {
      ...
      messages: (await db.query(`
        SELECT 
          ...
          m.is_system_message,
          CASE 
            WHEN m.sender_id = $1 THEN o.name
            ELSE u.name
          END as sender_name,
          CASE 
            WHEN m.receiver_id = $1 THEN o.name
            ELSE u.name
    ...
    if (format === 'csv') {
      ...
      return res.send(JSON.stringify(organizationData, null, 2));
    }
  } catch (error) {
